...
describe('TenantService', () => {
  ...
  describe('createTenant', () => {
    it('should create and return a tenant', async () => {
      ...
      expect(tenantRepository.findOne).toHaveBeenCalledWith({ where: { domain: dto.domain } });
    ...
    it('should throw BadRequestException if tenant already exists', async () => {
      ...
      await expect(tenantService.createTenant(dto)).rejects.toThrow(BadRequestException);
      expect(tenantRepository.findOne).toHaveBeenCalledWith({ where: { domain: dto.domain } });
    });
  });
  ...
  describe('findById', () => {
    it('should return a tenant by id', async () => {
      const tenant = { id: '1', name: 'Test Tenant', domain: 'test.com', users: [] };
      (tenantRepository.findOne as jest.Mock).mockResolvedValue(tenant);
      
      const result = await tenantService.findById('1');
      expect(result).toEqual(tenant);
      expect(tenantRepository.findOne).toHaveBeenCalledWith({ where: { id: '1' }, relations: ['users'] });
    });

    it('should throw NotFoundException if tenant is not found', async () => {
      (tenantRepository.findOne as jest.Mock).mockResolvedValue(null);
      
      await expect(tenantService.findById('1')).rejects.toThrow(NotFoundException);
      expect(tenantRepository.findOne).toHaveBeenCalledWith({ where: { id: '1' }, relations: ['users'] });
    });
  });
  ...
  describe('findByDomain', () => {
    it('should return a tenant by domain', async () => {
      ...
      expect(result).toEqual(tenant);
      expect(tenantRepository.findOne).toHaveBeenCalledWith({ where: { domain: 'test.com' }, relations: ['users'] });
    });
    ...
    it('should return null if domain is empty or tenant is not found', async () => {
      ...
      expect(tenantRepository.findOne).toHaveBeenCalledWith({ where: { domain: 'unknown.com' }, relations: ['users'] });
    });
  });
  ...
  describe('getAllTenants', () => {
    it('should return all tenants', async () => {
      ...
      expect(tenantRepository.find).toHaveBeenCalledWith({ relations: ['users'] });
    });
  });
});
